...
public class OmniBotV20 extends AdvancedRobot {
    ...
    private static final double MAX_SHOOTING_DISTANCE = 250; // Distância máxima para atirar
    ...
    public void onScannedRobot(ScannedRobotEvent e) { // Se escaneia inimigo, pega os dados dele, começa a mover e atirar
        ...
        // Verifica se o inimigo está perto o suficiente para atirar
        if (e.getDistance() <= MAX_SHOOTING_DISTANCE) {
    ...
    public void onHitByBullet(HitByBulletEvent e) { // se recebe um tiro, muda a direção e segue p ela
        direction *= -1;
        setTurnRight(90 - e.getBearing());
        setAhead(100 * direction);
    }

    public void onHitWall(HitWallEvent e) { // se bate no muro, muda a direção e vai para trás
        direction *= -1;
        setBack(50);
        setTurnRight(90);
    }

    public void onHitRobot(HitRobotEvent e) { // se bate num robô, mira nele e volta para trás
    ...
    static class EnemyData { // onde armazenamos os dados do inimigo
        double energy;
        double lastBearing;
        double distance;
        long lastSeen;
